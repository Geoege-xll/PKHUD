...
open class PKHUD: NSObject {
    ...
    public static var titleFont: UIFont = UIFont.systemFont(ofSize: 15.0, weight: .bold)

    /// 默认主标题颜色（默认 .label 90% 不透明度）
    public static var titleColor: UIColor = UIColor.label.withAlphaComponent(0.9)

    /// 默认副标题字体（默认 14pt medium，清晰饱满）
    public static var subtitleFont: UIFont = UIFont.systemFont(ofSize: 14.0, weight: .medium)

    /// 默认副标题颜色（默认 .label 75% 不透明度，通透且高对比度）
    public static var subtitleColor: UIColor = UIColor.label.withAlphaComponent(0.75)

    /// 默认图标/动效主题色（默认 .label 85% 不透明度）
