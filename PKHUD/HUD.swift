...
public enum HUDContentType: Sendable {
    /// 动画成功对勾图标（支持无文本 `.success` 或带文本 `.success("保存成功")`）
    case success(String?)
    /// 动画错误叉号图标（支持无文本 `.error` 或带文本 `.error("网络连接失败")`）
    case error(String?)
    /// 旋转进度菊花图标（支持无文本 `.progress` 或带文本 `.progress("加载中...")`）
    case progress(String?)
    /// 自定义静态图片展示（支持传可选提示文本）
    case image(UIImage?, String?)
    /// 自定义自动连续旋转图片展示（支持传可选提示文本）
    case rotatingImage(UIImage?, String?)
    /// 系统 SF Symbol 矢量图标（支持传可选提示文本，如 `.systemImage(name: "heart.fill", title: "已收藏")`）
    ...
    // MARK: - 便捷静态属性（无文本极简调用，如 HUD.flash(.success)）
    ...
    /// 自定义静态图片提示（无文本便捷调用）
    public static func image(_ image: UIImage?) -> HUDContentType {
        return .image(image, nil)
    }

    /// 自定义旋转图片提示（无文本便捷调用）
    public static func rotatingImage(_ image: UIImage?) -> HUDContentType {
        return .rotatingImage(image, nil)
    }

    /// 系统 SF Symbol 矢量图标提示
    /// - Parameters:
    ///   - name: SF Symbol 图标名称
    ///   - title: 可选提示文本（默认为 nil）
...
public final class HUD {
    ...
    fileprivate static func contentView(_ content: HUDContentType) -> UIView {
        ...
        case let .rotatingImage(image, title):
            return PKHUDRotatingImageView(image: image, title: title)
